...
public class NettyTOMMessageDecoder extends ByteToMessageDecoder {
    ...
    private boolean dummyMACs;
    private NettyClientServerCommunicationSystemServerSide ncs;

    private org.slf4j.Logger logger = LoggerFactory.getLogger(NettyTOMMessageDecoder.class);


    public NettyTOMMessageDecoder(boolean isClient, Map sessionTable, int macLength, ViewController controller, ReentrantReadWriteLock rl, int signatureLength,
    		boolean useMAC, boolean dummyMACs, NettyClientServerCommunicationSystemServerSide ncs) {
    	this.ncs = ncs;
...
//                    logger.info("PUT INTO SESSIONTABLE - [client id]:"+sm.getSender()+" [channel]: "+cs.getChannel());
                    ...
                    ncs.registerClient( sm.getSender() );
    ...
    boolean verifyMAC(int id, byte[] data, byte[] digest) {
        ...
        rl.readLock().unlock();
        byte[] mac = macReceive.doFinal(data);
        boolean result = Arrays.equals(mac, digest);
        //long duration = System.nanoTime() - startInstant;
        //st.store(duration);
        return result || dummyMACs;
    }
