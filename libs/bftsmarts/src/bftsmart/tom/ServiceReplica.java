...
public class ServiceReplica {
	...
	private ServerViewController[] SVControllerList;
	...
	public ServiceReplica(int id, String configHome, boolean isToJoin, Executable executor, Recoverable recoverer) {
		...
		int numaddrs = new TOMConfiguration(id, configHome, "").getNumberOfAddresses();

		SVControllerList = new ServerViewController[ numaddrs ];
		for( int i=0; i<SVControllerList.length; i++ )
		{
			String hostfn;
			...
			if( i==0 )
				...
				hostfn = "hosts" + ( i+1 ) + ".config";

			SVControllerList[ i ] = new ServerViewController(id, configHome, hostfn);
		}
		SVController = SVControllerList[ 0 ];
	...
	private void init() {
		try {
			cs = new ServerCommunicationSystem(SVControllerList, this);
		} catch (Exception ex) {
		...
		for( int i=0; i<SVControllerList.length; i++ )
		{
			ServerViewController svctl = SVControllerList[ i ];
			boolean islast = i+1==SVControllerList.length;

			if (svctl.isInCurrentView()) {
				System.out.println("In current view: " + svctl.getCurrentView());
				if( islast )
					initTOMLayer(-1, -1); // initiaze the TOM layer
			} else {
				System.out.println("Not in current view: " + svctl.getCurrentView());
				if (this.isToJoin) {
					System.out.println("Sending join: " + svctl.getCurrentView());
					// Not in initial view. Will perform a join;
					int port = svctl.getStaticConf().getServerToServerPort(id) - 1;
					String ip = svctl.getStaticConf().getServerToServerRemoteAddress(id).getAddress().getHostAddress();
					...
					System.out.println("Waiting for the TTP: " + svctl.getCurrentView());
	...
	public void joinMsgReceived(VMMessage msg) {
		...
		if (r.getView().isMember(id)) {
			for( ServerViewController svctl : SVControllerList )
				svctl.processJoinResult(r);
