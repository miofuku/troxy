...
public abstract class DefaultRecoverable implements Recoverable, BatchExecutable {
    ...
    private static final java.util.logging.Logger logger = java.util.logging.Logger.getLogger( DefaultRecoverable.class.getCanonicalName() );
    ...
    private byte[][] executeBatch(byte[][] commands, MessageContext[] msgCtxs, boolean noop) {
        ...
        if (checkpointIndex == -1) {
			...
			if(secondHalf.length > 0) {
				...
				System.arraycopy(msgCtxs, checkpointIndex+1, secondHalfMsgCtxs, 0,  commands.length - (checkpointIndex + 1));
			} else
				firstHalfEids = eids;
			...
			if (!noop) {
			    stateLock.lock();
			    firstHalfReplies = appExecuteBatch(firstHalf, firstHalfMsgCtxs);
			    stateLock.unlock();
			}

//	        if ((eid > 0) && ((eid % checkpointPeriod) == 0)) {
			if( logger.isLoggable( Level.FINE ))
				logger.fine( "(DefaultRecoverable.executeBatch) Performing checkpoint for consensus " + eid);
            stateLock.lock();
...
//	        	System.out.println("----THERE IS A SECOND HALF----");
                ...
                if (!noop) {
                    stateLock.lock();
                    secondHalfReplies = appExecuteBatch(secondHalf, secondHalfMsgCtxs);
                    stateLock.unlock();
                }
    ...
    private int eidPosition(int[] eids, int eid) {
		...
		if( logger.isLoggable( Level.FINER ))
			logger.finer("--- Checkpoint is in position " + index);
        return index;
    }
