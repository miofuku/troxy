...
public class RSAKeyPairGenerator {
     ...
     */
    public void run(int id, String cfgdir) throws Exception {
        KeyPairGenerator keyGen = KeyPairGenerator.getInstance("RSA");
        keyGen.initialize(1024);
        KeyPair kp = keyGen.generateKeyPair();
        PublicKey puk = kp.getPublic();
        PrivateKey prk = kp.getPrivate();
        saveToFile(id,puk,prk,cfgdir);
    }
    
    private void saveToFile(int id, PublicKey puk, PrivateKey prk, String cfgdir) throws Exception {
        String path = cfgdir+System.getProperty("file.separator")+"keys"+
    ...
    public static void main(String[] args){
        try{
            new RSAKeyPairGenerator().run(Integer.parseInt(args[0]), args.length>1 ? args[1] : "config");
        }catch(Exception e){
